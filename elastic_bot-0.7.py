...
import tinysegmenter #сегментатор для японского
import nltk
...
es=Elasticsearch()
# elasticsearch-7.6.1$ sudo bin/elasticsearch-plugin install analysis-kuromoji - это для установки куромоджи-плагина
# в эластик
token = '' #здесь токен
...
# создает объект сегментатора для японского
segmenter = tinysegmenter.TinySegmenter()



# это функция, которая смотрит на сообщение, и в зависимости от его содержания,
    ...
    if len(upd) > 0:
        ...
        try:
            ...
            if my_message.text != 'NoneType' and '/tokenize_japanese@tokenize_sent_bot' in my_message.text.lower():
                result_dict = {}
                # берем из апдейтов телеграма последнее сообщение
                my_chat_id = tb.get_updates()[-1].message.chat.id
                try:
                    # оттуда же берем отправителя(имя пользователя)
                    my_sender = tb.get_updates()[-1].message.json['from']['username']
                except Exception as e:
                    print(e)
                    # если нет, будет анонимус
                    my_sender = 'anonymous'
                # оттуда же берется дата, и разбивается на время и дату
                ...
                # всё собирается в соответствующи маппингу словарь, потом отправляется в эластик
...
### Инфо
            ...
            # фильтр по времени
                ...
                print(msgs)
                #tb.send_message(my_chat_id, str(msgs))
                for i in msgs['hits']['hits']:
                    print(i)
                    tb.send_message(my_chat_id, "Результат поиска:")
                    tb.send_message(my_chat_id, 'время сообщения - ' + str(i['_source']['msg_time']))
                    tb.send_message(my_chat_id, 'чат айди- ' + str(i['_source']['chat_id']))
                    tb.send_message(my_chat_id, 'дата - ' + str(i['_source']['msg_date']))
                    tb.send_message(my_chat_id, 'предложение - ' + str(i['_source']['msg']))
                    tb.send_message(my_chat_id, 'токены - ' + str(i['_source']['tokenized_msg']))
            ...
            # Корейский
                ...
                for i in sender_msg['hits']['hits']:
                    ...
                    tb.send_message(my_chat_id, 'Запись удалена')
                         ...
                         my_message.text.lower():
                my_chat_id = tb.get_updates()[-1].message.chat.id
                msg_targ = tb.get_updates()[-1].message.text.split('/korean_search_msg@tokenize_sent_bot ')[1]
...
def main():
    ...
    while False:
        time.sleep(5)
        try:
            # с каждым новым сообщением в телеграм-апи копится буфер (он хранит всё, но в пределах какого-то окна)
            # чтобы оставаться в пределах окна, надо его двигать (это криво, но вроде работает)
    ...
    while True:
        ...
        try:
            ...
            print("errors2")
        try:
            # если не предыдущее, значит пришло новое
            if updates[-1].message.message_id != old_id:
                #print(len(updates))
                # новое сообщение затем становится предыдущим
                old_id = updates[-1].message.message_id
                try:
                    # потом запускается функция высылки в чат и обработки сообщения, которая выше; туда передаются
                    # обновления телеграма.
